...
#define LCM_FW_FIRST_CH		0 // Moiti� pr�cise de LCM_TOT_CH
...
class LCD {
	public:
	LCD(volatile uint8_t*, volatile uint8_t*);
	~LCD(void);
	LCD& put(const char);
	...
	LCD& operator<<(const char*);
	LCD& operator<<(const uint16_t);
	LCD& operator<<(const int16_t);
	LCD& operator<<(const char);
	const char* operator=(const char*);
	LCD& operator[](const uint8_t index);
