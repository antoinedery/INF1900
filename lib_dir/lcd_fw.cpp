 ...
 */
LCD::LCD(volatile uint8_t* ddr, volatile uint8_t* port) : _last_index(0),
 ...
 */
void LCD::write(const char* msg, const uint8_t index, const bool cb) {
	if (msg == NULL) {
		return;
	}
 ...
 */
void LCD::en_blink(const bool state) {
 ...
 * @param state		Nouvel �tat
 */
void LCD::en_cur(const bool state) {
 ...
 */
void LCD::set_bc_index(const uint8_t index) {
 ...
 *			premier entier)
 */
void LCD::build_cc(const uint8_t index, const uint8_t* rows) {
 ...
 * @return	Auto-r�f�rence
 */
LCD& LCD::operator<<(const char* msg) {
	if (msg == NULL) {
		return *this;
	}
 ...
 * @return	Auto-r�f�rence
 */
LCD& LCD::operator<<(const uint16_t u) {
 ...
 * @return	Auto-r�f�rence
 */
LCD& LCD::operator<<(const int16_t i) {
 ...
 */
const char* LCD::operator=(const char* msg) {
	if (msg == NULL) {
		return msg;
	}
 ...
 * @return		Auto-r�f�rence
 */
LCD& LCD::operator[](const uint8_t index) {
	if (index >= LCM_FW_TOT_CH) {
		return *this;
	}
